    ...
    public static class CaptureHelper
    {
        public static void SetPossibleCapture(FieldState[,] boardState, GameState gameState)
        {
            ...
            for (var i = 0; i < 8; i++)
                for (var j = 0; j < 8; j++)
                {
                    CapturePawns(boardState, 0, i, j, gameState);
                    //MoveDames(i, j, fieldState);
                }
        }

        private static void CapturePawns(FieldState[,] boardState, int counter, int x, int y, GameState gameState)
        {
            if (boardState[x, y] != gameState.CurrentPawn) return;
            CapturePawn(boardState, counter, x, y, 1, 1, gameState);
            CapturePawn(boardState, counter, x, y, 1, -1, gameState);
            CapturePawn(boardState, counter, x, y, -1, -1, gameState);
            CapturePawn(boardState, counter, x, y, -1, 1, gameState);
        }

        private static void CapturePawn(FieldState[,] boardState, int counter, int currentX, int currentY, int moveX, int moveY, GameState gameState)
        {
            ...
            if (
                (boardState[currentX, currentY] != gameState.CurrentPawn) ||
            ...
            {
                if (counter > 0) gameState.PossibleCapture.Add(new Capture(counter, boardState));
                return;
            }
            ...
            CapturePawns(tmpBoard, ++counter, newX, newY, gameState);
        }         
    }
}